(function(){
  ...
  const STATE_KEYS=['cozy_blackboard_answers','cozy_orchard_seeds','cozy_orchard_topics','cozy_notice_requests','cozy_trips','cozy_trip_reflections','cozy_heart_entries','cozy_hollow_buried_media','cozy_memory_events','cozy_global_butler_history','cozy_toolbox_local_items','cozy_notice_links','cozy_notice_chest','cozy_butler_watch_topics','cozy_butler_local_sources'];
  let busy=false;
  ...
  function installStyles(){
    ...
    style.textContent=`
    .cozy-butler-launcher{position:fixed;left:16px;top:16px;z-index:90;width:46px;height:46px;border:1px solid rgba(104,74,45,.22);border-radius:50%;padding:0;background:#fff8ec url("${prefix}assets/estate/butler_dog.png") center/cover no-repeat;box-shadow:0 8px 24px rgba(51,35,22,.2);cursor:pointer}.cozy-butler-launcher:after{content:"";position:absolute;right:1px;bottom:2px;width:10px;height:10px;border-radius:50%;background:#a99783;border:2px solid #fff8ec}.cozy-butler-launcher.online:after{background:#6d9b68}.cozy-butler-launcher.steward:after{background:#d5a53d}
    .cozy-butler-drawer{position:fixed;left:16px;top:16px;bottom:16px;z-index:100;width:min(390px,calc(100vw - 32px));display:none;grid-template-rows:auto 1fr auto;background:rgba(255,251,244,.97);color:#45372c;border:1px solid rgba(104,74,45,.18);border-radius:18px;box-shadow:0 24px 70px rgba(37,25,17,.3);overflow:hidden;backdrop-filter:blur(14px);font-family:"PingFang SC","Microsoft YaHei",sans-serif}.cozy-butler-drawer.open{display:grid}.cozy-butler-head{display:flex;align-items:center;gap:10px;padding:13px 14px;background:#f4e7d4;border-bottom:1px solid rgba(104,74,45,.12)}.cozy-butler-avatar{width:36px;height:36px;border-radius:50%;background:url("${prefix}assets/estate/butler_dog.png") center/cover no-repeat;flex:0 0 auto}.cozy-butler-title{min-width:0;flex:1}.cozy-butler-title strong{display:block;font-size:14px}.cozy-butler-state{display:block;font-size:11px;color:#887361;margin-top:2px}.cozy-butler-close{width:34px;height:34px;border:0;border-radius:50%;background:rgba(255,255,255,.55);font-size:20px;color:#735f4d;cursor:pointer}
  ...
  async function refreshStatus(){const launcher=document.querySelector('.cozy-butler-launcher');if(RUNTIME.readOnly){launcher?.classList.remove('online','steward');setState('公开预览 · 不读取私人数据');return;}try{const data=await api('/api/status');launcher?.classList.add('online');launcher?.classList.toggle('steward',!!data.steward_mode);setState(data.steward_mode?'掌院权限已开启 · 永久':'普通权限 · 已连接');}catch(e){launcher?.classList.remove('online','steward');setState('服务未连接');}}
  async function send(){
