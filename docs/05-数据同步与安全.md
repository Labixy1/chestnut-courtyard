...
| 云端生成图片与视频 | 私有 R2 `COZY_MEDIA` | 本机生成目录 | 不保存 | 不共享主人媒体 |
| 手动上传照片 | 私有 R2 `COZY_MEDIA`，KV 保存归档关系 | `assets/*/uploads/` | 不保存 | 不提供私人上传 |
| 备份 | `COZY_BACKUP` KV | 手动导出到临时文件 | 不保存 | 独立 KV |

结论：结构化数据以云端 KV 为主；云端生成媒体和手动上传到照片墙、旅行、树洞的照片都进入私有 R2，可在登录主人版的手机和电脑间读取。R2 文件没有公开直链，读取仍受主人会话保护。
...
记忆使用独立键：`memory:events`、`memory:sealed`、`memory:profile`、`memory:categories`、`memory:overrides`、`memory:distillation`、`memory:forgotten`。其中 `memory:forgotten` 保存彻底遗忘的 tombstone，必须和正文记忆一起备份、迁移和恢复，防止旧设备副本复活。
...
| 限制 | 影响 | 建议 |
| --- | --- | --- |
| R2 免费空间有限 | 云端媒体达到上限后无法继续保存 | 默认 1K 图片、480p/5 秒视频；9GB 停止写入并人工清理 |
| 手动照片误删或 R2 空间不足 | 归档中的图片可能无法读取，达到上限后不能继续上传 | 9GB 停止写入；删除前确认并定期核对媒体空间状态 |
